...
def file_there(flow, folder_id, filename):
    ...
    try:
        ...
        # pylint: disable=maybe-no-member
        results = service.files().list(
            q=f"'{folder_id}' in parents",          # man the query needs to have folder name/id in ''
            pageSize=10, fields="nextPageToken, files(id, name)").execute()
...
def upload_to_folder(flow, folder_id, file):
    ...
    file_id = file_there(flow, folder_id, file_name)
    ...
    # create drive api client
        ...
        if file_id == 1:
            ...
            # update the file content given the fiel_id
